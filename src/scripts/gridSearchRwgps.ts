 ...
 */
function convertToRwgpsBbox(bboxPolygon: GeoJSON.Feature<GeoJSON.Polygon>): BoundingBox {
...
async function searchBbox(
  ...
  try {
    ...
    while (hasMorePages && pageCount < 100) {
      ...
      if (hasMorePages) {
        console.log(`Cell ${index+1}: More results available, next page available`);
      }
    }
    ...
    const cellAvgDistance = allTrips.length > 0 
      ? cellTotalDistance / allTrips.length / 1000 
      : 0;
    
    console.log(`Cell ${index+1}: Found ${totalCount} trips, fetched ${allTrips.length}, avg distance: ${cellAvgDistance.toFixed(2)} km`);
    ...
    console.error(`Error searching cell ${index+1}:`, error);
    return {
      cellIndex: index,
      tripsCount: 0,
      totalDistance: 0,
...
async function gridSearchRwgps(maxAreaKm2: number = 200, concurrency: number = 10) {
  ...
  console.log(`Searching ${bboxes.length} grid cells with max ${concurrency} concurrent requests...`);
  ...
  const avgTripDistance = totalTripsFound > 0 ? totalDistance / totalTripsFound / 1000 : 0;
