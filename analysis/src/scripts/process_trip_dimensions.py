...
def get_batch_filenames() -> List[str]:
    ...
    return glob.glob("/Volumes/T7/p/datasets/rwgps_100k_rides/trip_batches/*.msgpack")
...
def write_trip_dimensions(trip_dimensions: List[TripDimensions], output_filename: str) -> None:
    ...
    with open(output_filename, "wb") as f:
        # Serialize each TripDimensions object using its model_dump() method.
        f.write(msgpack.packb([trip.model_dump() for trip in trip_dimensions]))


def process_batch(batch_filename: str) -> None:
    # Derive output_filename by replacing 'trip_batch' with 'trip_dimensions' in the basename.
    base_name = os.path.basename(batch_filename)
    name_without_ext, _ = os.path.splitext(base_name)
    out_base_name = name_without_ext.replace("trip_batch", "trip_dimensions")
    output_filename = os.path.join("/Volumes/T7/p/datasets/rwgps_100k_rides/trip_dimensions", f"{out_base_name}.msgpack")
    
    if os.path.exists(output_filename):
        print(f"Output file {output_filename} already exists. Skipping processing for {batch_filename}.")
        return
